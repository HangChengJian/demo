<template>
  <Dialog v-model="dialogVisible" title="添加表头项">
...
const open = async () => {
  ...
  if(menuOptions.value.length === 0){
    menuOptions.value = await MenuApi.getTitle()
  }
...
const submitForm = async () => {
  ...
  //       ...(treeRef.value.getCurrentKey()) // 获得半选中的父节点

        
  //     ]
  //   }
  if(treeRef.value.getCheckedKeys().length == 0){
    message.warning('请选择添加项')
    return
  }
  const data = treeRef.value.getCheckedNodes().map(e=> {
    return{
      name:e.name,
